...
export default function VoiceprintOnboardingPage() {
  ...
  const createVoiceprint = async () => {
    ...
    try {
      ...
      const createdVoiceprintId = createData.voiceprint_id
      ...
      const computeResponse = await fetch('/api/voiceprint/compute', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          voiceprint_id: createdVoiceprintId
        })
      })
  ...
  const setAsDefaultAndContinue = async () => {
    ...
    try {
      // In a real implementation, you'd call an API to set as default
      console.log('Setting voiceprint as default:', voiceprintId)
      console.log('Telemetry: vp_set_default')
