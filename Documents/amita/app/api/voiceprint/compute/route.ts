...
interface ComputeVoiceprintRequest {
  voiceprintId: string
}

export async function POST(request: NextRequest) {
  console.log('Voiceprint computation API called')
  
  try {
    const body: ComputeVoiceprintRequest = await request.json()
    const { voiceprintId } = body
    
    if (!voiceprintId) {
      return NextResponse.json(
        { error: 'Voiceprint ID is required' },
        { status: 400 }
      )
    }
