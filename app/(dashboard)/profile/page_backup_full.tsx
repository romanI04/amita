...
import { VoiceFingerprint, VoiceLocks, ProtectionScore } from '@/components/voice'
import { 
  UserCircleIcon, 
  ChartBarIcon, 
  CheckCircleIcon,
...
export default function ProfilePage() {
  ...
  const handleToggleLock = async (lockId: string) => {
    ...
    if (lockType) {
      setSavingSettings(prev => ({ ...prev, [lockId]: true }))
      ...
      try {
        ...
        showToast(`${lockId.replace(/_/g, ' ')} ${newValue ? 'enabled' : 'disabled'}`, 'success')
      } catch (error) {
        showToast('Failed to update lock setting', 'error')
      } finally {
        setSavingSettings(prev => ({ ...prev, [lockId]: false }))
      }
    }
  }
      ...
      <AppLayout>
        ...
        {!profileData.voiceprint ? (
          ...
          <div className="max-w-7xl mx-auto py-8 px-6">
                    ...
                    {voiceLocks.map(lock => (
                      <div key={lock.id} className="flex items-center justify-between p-3 rounded-lg border border-gray-200">
                        ...
                        <button
                          ...
                          disabled={savingSettings[lock.id]}
                          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
                            ...
                            savingSettings[lock.id] ? 'opacity-50 cursor-not-allowed' : ''
                          }`}
                        >
                          <span
                            className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                              ...
                              savingSettings[lock.id] ? 'animate-pulse' : ''
                            }`}
                          />
                        </button>
                      </div>
            ...
            <div className="bg-white rounded-2xl p-6 mb-8 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                ...
                <Button 
                  ...
                  variant="outline"
                  onClick={async () => {
                    setSavingSettings(prev => ({ ...prev, defaultDomain: true }))
                    try {
                      // Save default domain to profile
                      await new Promise(resolve => setTimeout(resolve, 500))
                      showToast(`Default domain set to ${voiceProfileState.domains.active}`, 'success')
                    } catch (error) {
                      showToast('Failed to save default domain', 'error')
                    } finally {
                      setSavingSettings(prev => ({ ...prev, defaultDomain: false }))
                    }
                  }}
                  disabled={savingSettings.defaultDomain}
                >
                  {savingSettings.defaultDomain ? 'Saving...' : 'Set default domain'}
                </Button>
              </div>
            ...
            {/* 7. Privacy & Control */}
            <div className="bg-white rounded-2xl p-6 shadow-sm">
              <h3 className="text-lg font-semibold text-gray-900 mb-6">Privacy & Control</h3>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  ...
                  <button
                    onClick={async () => {
                      const settingKey = 'analytics'
                      setSavingSettings(prev => ({ ...prev, [settingKey]: true }))
                      try {
                        await new Promise(resolve => setTimeout(resolve, 300))
                        showToast('Analytics preference updated', 'success')
                      } catch (error) {
                    ...
                    disabled={savingSettings.analytics}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 bg-gray-200 ${
                      savingSettings.analytics ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                  >
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform translate-x-1 ${
                      savingSettings.analytics ? 'animate-pulse' : ''
                    }`} />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Receive updates about voice drift and improvements</p>
                  </div>
                  <button
                    onClick={async () => {
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 bg-green-600 ${
                      savingSettings.notifications ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                  >
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform translate-x-6 ${
                      savingSettings.notifications ? 'animate-pulse' : ''
                    }`} />
                  </button>
                </div>
                ...
                <div className="border-t pt-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-gray-700 mb-2">Data Management</p>
                      <p className="text-sm text-gray-500">
                        Your samples are encrypted and never used to train models
                      </p>
                    </div>
                    ...
                    <div className="flex space-x-3">
                      <Button 
                        ...
                        size="sm"
                        onClick={async () => {
                          setSavingSettings(prev => ({ ...prev, export: true }))
                          try {
                            ...
                            setSavingSettings(prev => ({ ...prev, export: false }))
                          }
                        }}
                        disabled={savingSettings.export}
                      >
                        {savingSettings.export ? (
                          <>
                      ...
                      <Button 
                        ...
                        size="sm"
                        onClick={() => {
                          if (confirm('Are you sure you want to delete all samples? This action cannot be undone.')) {
                            setSavingSettings(prev => ({ ...prev, delete: true }))
                            setTimeout(() => {
                              showToast('Samples deletion initiated', 'success')
                              setSavingSettings(prev => ({ ...prev, delete: false }))
                            }, 1000)
                          }
                        }}
                        disabled={savingSettings.delete}
                        className="hover:bg-red-50 hover:text-red-600 hover:border-red-300"
                      >
                        {savingSettings.delete ? (
                          <>
                            <ArrowPathIcon className="h-4 w-4 mr-2 animate-spin" />
                            Deleting...
                          </>
